...
public class Ex06 {
	...
	/* 
	  ...
	  3. 클래스 구조 : 헤더, 멤버필드, 멤버메서드, 생성자 
	    1) 헤더 : [접근제한자] [클래스종류] class 클래스이름{}              //public [생략됨] class Ex06      
	      가) 접근제한자 : 클래스, 멤버필드, 멤버메서드, 생성자 모두 맨 앞에 사용 가능 {생략가능}
	      ...
	      나) 클래스 종류 : 일반적인 클래스는 생략한다.              
	                     특정 클래스에 해당하는 클래스에게만 예약어를 사용한다. ex) final, abstract etc...
	  	  다) class : 클래스임을 나타내는 예약어
	  	  라) 클래스이름 : 파일이름 = 저장이름 = 생성자 이름 
	  	                첫글자는 대문자, 두단어 이상일 때는 단어 첫글자들은 대문자 
	  	                숫자는 중간이나 뒤에 사용가능, 공백문자 사용 불가
	  ...
	  5. 멤버 메서드 : 동작, 기능
	    ...
	    1) 메서드 구성
	 		...
	 		나) 메서드 종류 : 일반적인 메서드는 이 부분 생략한다. 
           	...
           	라) 메서드 이름 : 첫글자 소문자, 두 단어 이상일때 단어 첫 글자들은 대문자
           	               ([인자=매개변수])앞에 이름 붙는다. =>> main 
           	               
           	마) 인자 = 매개변수 : 매서드가 실행할 때 필요한 정보를 외부에서 받기 위한 통로 ex)=>(String[] args) 
	   		   ** 매서드이름이 같아도 인자 수나 인자 자료형이 다르면 다른 메서드로 취급한다. 
	   		   ** 오버로딩(중복정의) : 클래스 안에 같은 이름을 가진 메서드가 여러개 정의 되어 있는 것
	   6. 생성자 : 클래스가 객체로 만들어 질 때 한번 호출 된다. 
	     1) 목적 : 멤버필드(변수,상수)의 초기값을 지정하는 것이 목적이다. 
	     2) 형식 : 클래스 이름과 생성자 이름은 같다. 
	              메서드와 다른점은 반환형이 없다. 
	              [접근제한자] 생성자이름(=클래스이름)([인자]){실행내용} // 실행내용이 대부분 멤버필드의 초기값을 지정한다. 
	              
	              public class Ex06 이때 생성자는 [접근제한자]Ex06([인자]){}
	              인자가 없는 생성자를 "기본생성자" 라고 한다. => Ex06(){} 
				  생성자도 오버로딩이 가능하다.
				  즉, 하나의 클래스 안에 여러개의 생성자가 존재할 수 있다. 
	     3) 생성자가 없이 클래스를 객체로 생성하면 기본생성자를 호출해서 객체를 만든다.
	     4) 모든 클래스는 생성자를 가지고 있다. 
	     
	     5) 클래스를 이용해서 객체를 만드는 방법
	     Scanner scan  =        new           Scanner(System.in);
	      클래스  참조변수 = 객체를 만들기 위한예약어     생성자(인자)
	     new 예약어를 사용하면 무조건 heap 메모리에 사용영역을 만든다. 
	     즉, instance, 객체 생성을 의미한다. 
	     
	   7. 클래스를 객체로 만든 후 멤버필드와 멤버메소드에 접근하는 방법    
	      : 참조변수.멤버필드, 참조변수.멤버메서드()      (단, 접근제한자를 확인하자. private 면 접근할 수 없다.)
	 
	   8. static 과 instance : 멤버필드, 멤버메서드에 공통으로 해당되는 사항이다. 
	     1) instance(인스턴스) : 객체 생성
	        - 인스턴스필드, 인스턴스메서드 : 객체 생성할 때 같이 생성되는 필드와 메서드를 말한다. 
	        - 일반적인 클래스의 멤버필드나 멤버메서드는 모두 인스턴스이다. 
	        - 호출방법 : 참조변수.멤버필드, 참조변수.멤버메서드()
	        - heap 메모리에 만들어진다. 
	     
	     2) static(스태틱) 
	        - 객체 생성과 상관없이 미리 만들어진 멤버필드와 멤버메서드를 말한다.
