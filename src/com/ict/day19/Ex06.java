...
/*
	   File 클래스 : 특정위치에 존재하는 파일이나 디렉토리(폴더)를 처리하는 클래스
	   	  - 주요 생성자 : File(String 경로), File(String 상위경로), File(String 하위경로)
	   	                                  File(File 상위경로, String 하위경로)
	      - 주요 메서드 : creatNewFile()    : 파일 생성
	      			   ...
	      			   exists()          : 있으면 true
	      			   getName()         : 파일 이름 반환
	      			   length()          : 파일 크기를 byte 로 변환(directory 는 크기가 없음 **0이 아니다.)
	      			   list()            : 특정위치의 내용을 String 배열로 만들어서 저장 
	      			   getAbsolutePath() : 절대 주소
	      			   getPath()         : 상대주소
...
public class Ex06 {
	public static void main(String[] args) {
		// 본인 환경에 맞게 경로 설정
		//String pathname = "D:\\chulhwan\\javastudy";
		String pathname = "D:/chulhwan/javastudy";
